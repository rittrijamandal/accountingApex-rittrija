...
export default function PublishedWorlds() {
  ...
  function launchWorkspace(world: CurriculumLobbyWorld) {
    navigate(
      `/grader/workspace?id=${world.id}&noRun=1&returnTo=${encodeURIComponent("/expert/worlds")}`,
    );
  }
  ...
  return (
    <AppShell sidebar={false}>
      <div className="px-8 pt-8 pb-2">
        ...
        <div className="label-eyebrow">Published Worlds</div>
        ...
        <p className="text-sm text-slate-500 mt-2">
          {loading
            ? "Loading…"
            : `${worlds.length} curriculum benchmark${worlds.length !== 1 ? "s" : ""} — same library as the Grader Lobby: Audit, Invoice Approval, and Deals & Advisory.`}
        </p>
      </div>
      ...
      <div className="px-8 py-6">
        ...
        {loading && worlds.length === 0 ? (
          ...
          <div className="rounded-3xl bg-white shadow-sm p-12 text-center">
            <p className="font-serif-display text-2xl text-slate-400 italic">No curriculum worlds available.</p>
            <p className="text-sm text-slate-400 mt-2">
              Publish worlds titled <strong>Audit</strong> and <strong>Invoice Approval</strong> (and optionally{" "}
              <strong>Deals & Advisory</strong>). Deals &amp; Advisory uses the built-in sample when no matching row exists.
            </p>
          </div>
        ) : (
