...
function render(app, dbRow, state, isAdmin) {
  ...
  app.innerHTML = `
    ...
    <section class="section" id="sec-data-room">
      ...
      <p class="muted" style="margin-bottom:14px">Upload source documents, classify them, and mark any misleading files. Graders see exactly what you assemble here.</p>
    ...
    <section class="section" id="sec-agent-rules">
      ...
      <div class="field">
        ...
        <textarea id="task-prompt" rows="6">${esc(state.taskPrompt)}</textarea>
      </div>
    ...
    <section class="section" id="sec-rubric-builder">
      ...
      <div class="inspector-placeholder" style="margin-bottom:14px">
        <h3>Example criterion (reference only)</h3>
        <div class="tiny-note">#1 · Type: det · Label: Verify total expense calculation</div>
        <div class="tiny-note">Notes: Check that the agent's final calculated total matches exactly $4,250.00 based on the provided Q3 receipts.</div>
      </div>
...
function renderBrowseDashboard(state) {
  ...
  if (!state.uploadedFiles.length) {
    return `${toolbar}<div class="empty-note">No files yet. Click <strong>Upload document</strong> to add the first one.</div>`;
  }
    ...
    <div class="doc-viewer-shell">
      ...
      <aside class="drawer-panel ${drawerClass}" aria-hidden="${state.drawerOpen ? 'false' : 'true'}">
        <div class="drawer-header">
          <h3>Details</h3>
          <button type="button" class="btn-icon" data-action="close-drawer" aria-label="Close">✕</button>
        </div>
...
function renderUploadMode(state) {
  ...
  return `
    <div class="inspector-placeholder">
      ...
      ${p.type === CUSTOM_TYPE_ID ? `<div class="field" style="margin-top:14px"><label>Custom file type</label><input type="text" data-bind-pending="customType" value="${esc(p.customType)}" placeholder="e.g., Vendor contract" /></div>` : ''}
      ...
      <label class="check-row">
        ...
        <span>Mark as misleading / red herring</span>
      </label>
      <div class="field" style="margin-top:14px">
...
function renderCsvGrid(file) {
  ...
  return `
    ...
    <button type="button" class="btn btn-ghost" style="margin-top:10px" data-action="add-grid-row">+ Add row</button>
  `;
}
  ...
  wrap.innerHTML = state.rubric
    .map(
      (r, i) => `<div class="rubric-row" data-rub-i="${i}">
        ...
        <button type="button" class="btn-icon" data-action="del-rubric" data-rub-i="${i}" aria-label="Remove">✕</button>
      </div>`
    )
    .join('');
}
...
async function init() {
  ...
  if (!id) {
    app.innerHTML = '<div class="wrap"><div class="err-banner">Missing world id. <a href="expert.html">Back to worlds</a></div></div>';
    return;
  }
  ...
  if (error || !row) {
    app.innerHTML = `<div class="wrap"><div class="err-banner">${esc(error?.message || 'World not found or no access.')}</div><p class="muted" style="margin-top:12px"><a href="expert.html">← Back to worlds</a></p></div>`;
    return;
  }
...
${esc(state.review.combined_notes || 'No reviewer notes provided.')}</div>
  ...
  function renderSubmitReviewPanel() {
    ...
    if (state.review.status === 'draft') {
      panel.innerHTML = `<p class="muted" style="margin-bottom:14px">When the world is ready, submit it for inter-annotator review.</p>
...
init().catch((err) => {
  ...
  app.innerHTML = `
    ...
    <p class="muted"><a href="expert.html">← Back to worlds</a></p>
  `;
});
