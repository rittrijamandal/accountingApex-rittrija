...
function badgeHtml(type) {
  ...
  const labels = { core: 'Core', warn: 'Legacy', noise: 'Noise' };
...
function renderGraderLobby() {
  const cards = getLobbyWorlds().map((w) => {
    return `<article class="grader-world-card">
      ...
      <button type="button" class="gen-btn grader-enter-btn" onclick='enterPublishedWorld(${JSON.stringify(w.id)})'>Open world</button>
    </article>`;
  }).join('');
  return `
    ...
    <div class="grader-lobby-inner">
      ...
      <p class="grader-lobby-sub">Pick a published world to open the grading workspace. You can switch between worlds at any time from the header.</p>
